...
// ==================== CONFIG ====================
const CONFIG = {
  GUILD_ID: process.env.GUILD_ID || 'COLOQUE_AQUI',
  // Canais
...
async function replyServerChoice(interaction, lang='pt') {
  const text = lang === 'es' ? 'Elige tu servidor para continuar:' : 'Escolha o servidor para continuar:';
  await interaction.reply({ content: `✅ ${text}`, components: [serverChoiceRow(lang)], ephemeral: true });
}
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      // Idioma
      if (id === 'lang_pt') {
        await interaction.reply({ content: '✅ Você selecionou **Português**.', ephemeral: true });
        return replyServerChoice(interaction, 'pt');
      }
      if (id === 'lang_es') {
        await interaction.reply({ content: '✅ Has seleccionado **Español**.', ephemeral: true });
        return replyServerChoice(interaction, 'es');
      }
      ...
      // Escolha PvE → posta um botão de uso único no canal indicado
      if (id === 'choose_pve') {
      ...
      if (id.startsWith('pve_btn_')) {
        ...
        // Abre modal
        const modal = new ModalBuilder().setCustomId(`modal_pve_${ownerId}`).setTitle('Cadastro PvE — Steam ID');
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith('modal_pve_')) {
        ...
        const userId = interaction.customId.split('modal_pve_')[1];
        ...
        // Desabilita o botão da mensagem (se possível: a interação veio do modal; não temos a referência da msg; então não editamos aqui)
        await interaction.reply({
